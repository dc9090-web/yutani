...
pub fn install_to(theme_dir: &Path, desktop: &Path, exec: &str) -> anyhow::Result<usize> {
    for (dir, name, bytes) in ICONS {
        let path = icon_path(theme_dir, dir, name);
...
pub fn uninstall_from(theme_dir: &Path, desktop: &Path) -> anyhow::Result<usize> {
    let mut removed = 0;
...
mod tests {
    ...
    fn dirs(tag: &str) -> Sandbox {
        ...
        let icons = root.join("icons").join("hicolor");
        let desktop = root.join("applications").join(DESKTOP_ID);
        Sandbox { root, icons, desktop }
    }

    fn symbolic(sandbox: &Sandbox) -> PathBuf {
        sandbox.icons.join(crate::assets::SYMBOLIC_DIR)
    }
    ...
    fn install_writes_every_icon_and_the_desktop_file_and_is_idempotent() {
        let s = dirs("install");
        assert_eq!(install_to(&s.icons, &s.desktop, "/opt/yutani-applet").unwrap(), 6);
        for (dir, name, bytes) in crate::assets::ICONS {
            assert_eq!(std::fs::read(s.icons.join(dir).join(name)).unwrap(), bytes);
        }
        assert!(std::fs::read_to_string(&s.desktop).unwrap().contains("Exec=/opt/yutani-applet"));
        // Running it again rewrites the same six files, not more.
        assert_eq!(install_to(&s.icons, &s.desktop, "/opt/yutani-applet").unwrap(), 6);
        assert_eq!(std::fs::read_dir(symbolic(&s)).unwrap().count(), 4);
        assert_eq!(
            std::fs::read_dir(s.icons.join(crate::assets::SCALABLE_DIR)).unwrap().count(),
            1
        );
    }
    ...
    fn the_launcher_icon_is_installed_outside_the_symbolic_directory() {
        let s = dirs("launcher");
        install_to(&s.icons, &s.desktop, "/opt/yutani-applet").unwrap();
        assert!(s.icons.join("scalable/apps/y-color.svg").is_file());
    ...
    fn uninstall_removes_exactly_our_files() {
        ...
        assert_eq!(uninstall_from(&s.icons, &s.desktop).unwrap(), 6);
        assert!(!s.desktop.exists());
    ...
    fn uninstall_on_a_clean_system_removes_nothing_and_does_not_fail() {
        let s = dirs("clean");
        assert_eq!(uninstall_from(&s.icons, &s.desktop).unwrap(), 0);
    }

    #[test]
    fn the_real_paths_are_the_xdg_user_ones() {
        assert!(theme_dir().ends_with("icons/hicolor"));
